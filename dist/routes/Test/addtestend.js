...
const Testend_js_1 = __importDefault(require("../../models/Testend.js"));
const Testqend_js_1 = __importDefault(require("../../models/Testqend.js"));
const router = express_1.default.Router();
router.get('/', async (req, res) => {
    try {
        const tests = await Testend_js_1.default.find();
...
router.get('/view', async (req, res) => {
    try {
        const tests = await Testqend_js_1.default.find().populate('testid');
        res.status(200).json({ tests: tests });
    }
    catch (error) {
        res.status(500).json({ message: "Internal server error. Try again after some time" });
...
router.post('/', async (req, res) => {
    ...
    if (req.body.status == "addTest") {
        let test = new Testend_js_1.default({
    ...
    //if for the updateTest
        ...
        try {
            await Testend_js_1.default.findByIdAndUpdate({ _id: req.body.id }, {
                testname: req.body.testname,
                testtype: req.body.testtype,
                testdate: req.body.testdate,
                testenddate: req.body.testenddate,
    ...
    //else if for delete the Test..
        ...
        try {
            await Testend_js_1.default.findByIdAndDelete({ _id: req.body.id });
