...
router.get('/', async (req, res) => {
    try {
        let data = await Testqend_1.default.find({});
...
router.post('/', async (req, res) => {
    ...
    if (req.body.status == "add") {
        ...
        try {
            let testq = new Testqend_1.default({
    ...
    //else for update endpoint
        ...
        console.log(req.body);
        try {
            let testq = await Testqend_1.default.findByIdAndUpdate({ _id: req.body.id }, {
                testname: req.body.testname,
                testid: req.body.testid,
                question1: req.body.question1,
                question2: req.body.question2,
    ...
    //else for delete endpoint
        ...
        try {
            let testq = await Testqend_1.default.findByIdAndDelete({ _id: req.body.id });
    ...
    //else for getting questions added
        ...
        if (req.headers['authorization'] == process.env.AUTH_KEY) {
            try {
                const data = await Testqend_1.default.findOne({ testid: req.body.testid });
                res.status(200).json({ data, success: true });
            }
            catch (err) {
                res.status(200).json({ success: false, message: "Something went wrong. Please try again later" });
