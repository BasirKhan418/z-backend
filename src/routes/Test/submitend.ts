...
let findTest = await Testqend.findById({_id:testid});
if(findTest==null){
...
let user =await User.findOneAndUpdate({email:decoded.email},{endtestResult:totalp,endappeartest:true,testcorrect:`${totalcorrect}/${total}`,endtestid:testid});
if(user==null){
