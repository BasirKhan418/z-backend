...
import Testend from '../../models/Testend.js';
import Testqend from '../../models/Testqend.js';
const router = express.Router();
router.get('/',async(req,res)=>{
    try{
        const tests = await Testend.find();
...
router.get('/view',async(req,res)=>{
    try{
        const tests = await Testqend.find().populate('testid');
        res.status(200).json({tests:tests});
    }
    catch(error){
        res.status(500).json({message:"Internal server error. Try again after some time"});
...
router.post('/',async(req,res)=>{
    ...
    //if for the post method
        ...
        //if for the updateTest
            ...
            try{
                await Testend.findByIdAndUpdate({_id:req.body.id},{
                    testname: req.body.testname,
                    testtype: req.body.testtype,
                    testdate: req.body.testdate,
                    testenddate: req.body.testenddate,
        ...
        //else if for delete the Test..
            ...
            try{
                await Testend.findByIdAndDelete({_id:req.body.id});
